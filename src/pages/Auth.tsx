...
const MATH_SYMBOLS = ['∑', '∫', 'π', '√', '∞', 'Δ', '∇', 'λ', 'θ', 'σ', '∂', 'μ'];
    ...
    <div className="min-h-screen bg-background flex flex-col overflow-hidden relative">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden>
        {MATH_SYMBOLS.map((sym, i) => (
          <motion.span
            key={i}
            className="absolute font-mono-code text-primary/10 font-bold select-none"
            style={{
              fontSize: `${1.2 + (i % 3) * 0.8}rem`,
              left: `${(i * 137.5) % 100}%`,
              top: `${(i * 97.3) % 100}%`,
            }}
            animate={{ y: [0, -18, 0], rotate: [0, i % 2 === 0 ? 8 : -8, 0], opacity: [0.07, 0.18, 0.07] }}
      ...
      <div className="relative z-10 flex items-center justify-between px-4 pt-4 pb-2">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => navigate('/')}
          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-card border border-border text-foreground hover:bg-secondary transition-colors text-sm font-semibold"
        >
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground hidden sm:inline font-mono-code">locale:</span>
          <LanguageSelector locale={locale} onChangeLocale={changeLocale} />
        </div>
      </div>

      {/* Main card */}
      <div className="relative z-10 flex-1 flex items-start sm:items-center justify-center px-4 pt-6 sm:pt-0 pb-8">
        <motion.div
        ...
        >
          ...
          {/* Math formula decorative strip */}
          <div className="mb-5 rounded-xl bg-card border border-border/60 px-4 py-2 overflow-hidden">
            <motion.p
              className="font-mono-code text-xs text-primary/70 text-center whitespace-nowrap"
              animate={{ x: [0, -40, 0] }}
              transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
            >
              ∇L(θ) = ∂/∂θ · E[log π(a|s)] · R &nbsp;·&nbsp; P(X=x) = e^(-λ)λˣ/x! &nbsp;·&nbsp; det(A) = ∑ εᵢⱼₖ aᵢ₁aⱼ₂aₖ₃
            </motion.p>
          </div>
