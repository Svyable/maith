...
// XP level thresholds
const LEVELS = [
  { level: 1, xp: 0, title: 'Novice', emoji: '🌱' },
  { level: 2, xp: 100, title: 'Learner', emoji: '📚' },
  { level: 3, xp: 300, title: 'Scholar', emoji: '🎓' },
  { level: 4, xp: 600, title: 'Expert', emoji: '⚡' },
  { level: 5, xp: 1000, title: 'Master', emoji: '🏆' },
  { level: 6, xp: 2000, title: 'Grandmaster', emoji: '👑' },
  { level: 7, xp: 5000, title: 'Legend', emoji: '🔥' },
  { level: 8, xp: 10000, title: 'Mythic', emoji: '💎' },
];
...
function getLevel(xp: number) {
  ...
  const progress = next ? Math.min(((xp - current.xp) / (next.xp - current.xp)) * 100, 100) : 100;
  return { ...current, progress, nextXp: next?.xp ?? current.xp };
}
