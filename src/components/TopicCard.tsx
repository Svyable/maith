...
export function TopicCard({ topic, isSelected, isExplicitSelection, onToggle, selectedDifficulties }: TopicCardProps) {
  ...
  const canToggle = hasMatchingQuestions || isExplicitSelection;
  const showSelected = isExplicitSelection || (isSelected && hasMatchingQuestions);
  ...
  return (
    <motion.button
      ...
      className={`relative w-full min-h-20 p-4 rounded-lg border text-left transition-all focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
        !hasMatchingQuestions && !isExplicitSelection
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all shrink-0 ${
          showSelected ? 'border-primary bg-primary' : 'border-muted-foreground/30'
        }`}>
          {showSelected && <span className="text-primary-foreground text-xs">✓</span>}
        </div>
      </div>
    </motion.button>
  );
