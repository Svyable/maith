...
export function QuizHeader({ streak, showStreak }: QuizHeaderProps) {
  ...
  const goToQuiz = () => {
    if (location.pathname === APP_PATHS.home) {
      document.querySelector(QUIZ_SETUP_HASH)?.scrollIntoView({ behavior: "smooth" });
      return;
    }
    navigate(`${APP_PATHS.home}${QUIZ_SETUP_HASH}`);
  };
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-3 sm:px-4">
        ...
        <nav aria-label={t("nav.primary")} className={`${showStreak ? "" : "ml-auto"} hidden items-center gap-1 xl:flex`}>
          {PRIMARY_NAV_ITEMS.map((item) => {
            const active = isItemActive(item.path, "hash" in item ? item.hash : undefined);
            if ("hash" in item) {
              return (
                <Button key={item.id} variant="ghost" size="sm" onClick={goToQuiz} aria-current={active ? "page" : undefined} className={active ? "bg-primary/15 text-primary" : "text-muted-foreground"}>
                  <span aria-hidden="true">{item.emoji}</span>{t(item.labelKey)}
                </Button>
              );
            }
            return (
              <Button key={item.id} variant="ghost" size="sm" asChild className={active ? "bg-primary/15 text-primary" : "text-muted-foreground"}>
        ...
        <div className="hidden items-center gap-2 xl:flex">
          {utilityControls}
          <Button variant={user ? "secondary" : "default"} size={user ? "icon" : "sm"} onClick={() => navigate(user ? APP_PATHS.profile : APP_PATHS.auth)} aria-label={user ? t("nav.profile") : undefined} className={user ? "h-11 w-11" : "h-11"}>
          ...
          <Sheet>
            ...
            <SheetContent className="w-[min(88vw,22rem)] p-5">
              ...
              <nav aria-label={t("nav.mobilePrimary")} className="grid gap-1">
                <SheetClose asChild><NavLink to={APP_PATHS.home} className="flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold hover:bg-secondary"><span aria-hidden="true">⌂</span>{t("nav.home")}</NavLink></SheetClose>
                {PRIMARY_NAV_ITEMS.map((item) => (
                ...
                <a href={alphabet.url} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold hover:bg-secondary"><span aria-hidden="true">{alphabet.emoji}</span>{t(alphabet.navLabelKey)} ↗</a>
              </nav>
              <div className="mt-6 flex items-center gap-2 border-t border-border pt-5">
                {showStreak ? utilityControls : null}
                <SheetClose asChild>
                  <Button className="min-h-11 flex-1" variant={user ? "secondary" : "default"} onClick={() => navigate(user ? APP_PATHS.profile : APP_PATHS.auth)}>
