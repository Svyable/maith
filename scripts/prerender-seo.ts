...
function replaceRouteSchema(
  ...
  const topicCluster = getTopicReferenceCluster(pathname);
  const fieldCluster = getFieldReferenceCluster(pathname);
  const relatedLinks = topicCluster
    ? [...topicCluster.formulas, ...topicCluster.glossary, ...topicCluster.thinkers].map((link) => link.path)
    : fieldCluster
      ? [...fieldCluster.formulas, ...fieldCluster.glossary, ...fieldCluster.thinkers].map((link) => link.path)
      : getReferenceTopicBacklinks(pathname).map((link) => link.path);
